...
export default function OpenRequest (props) {
    ...
    let routeString = window.location.search.split("=")[1]

    useEffect(() => {
        if (routeString === "false") {
            setRoute(false)
        } else {
            setRoute(true)
        }
    ...
    const [fetchNewPathRoute, isLoadingNewPathRoute, errorNewPathRoute] = useFetching(async (id) => {
        const response = await PostService.getNewPathRoute(id)

        setNewPath(response)
    })

    const [fetchNewPathOrder, isLoadingNewPathOrder, errorNewPathOrder] = useFetching(async (id) => {
        const response = await PostService.getNewPathOrder(id)
    ...
    return (
        <div className={style.main} style={props.style}>
            {
                (!(!isLoadingOrderById || !isLoadingRouteById)) ?
                    ...
                    route ?
                            ...
                            <div className={style.block} style={props.addStyle}>
                                ...
                                <div className={style.posInfo}>
                                    <div className={style.info}>
                                        ...
                                        <MainInfoRequest route={route} openInfo={openInfo} setValFunc={setValues} values={values} edit={edit} openInfo={openInfo} allInfo={newRequest}/>
                                    </div>
                                    ...
                                    newRequest.orders?.isSingle ?
                                        ...
                                        props.newPath ? "" : (
                                            <div className={style.newPath}>
                                                ...
                                                {
                                                    isLoadingNewPathRoute ?
                                                        <Loading />
                                                        :
                                                        newPath.length > 0
                                                            ? newPath.map(item => (
                                                                <NewPath key={item.route.id} title={ReversRoutePoint(item.route)} routeId={item.route.id} mainRouteId={routerId}/>
                                                            ))
                                                            : (<NoneRequests />)
                                                }
                                            </div>)
                        ...
                        (<>
                            {
                                confirm ? <ModalConfirm setConfirm={openConfirm} routeId={newRequest.id} orders={newRequest}/> : ""
                            }
                            <div className={style.block} style={props.addStyle}>
                                {
                                    !props.buttonEdit
                                ...
                                {/*надо пофиксить при просмотре предложенных заявок. Роутинг*/}
                                <h4 className={style.title}>
                                    {newRequest?.isSingle ? (<div className={style.locked}></div>) : ''}
                                ...
                                <div className={style.posInfo}>
                                    <div className={style.info}>
                                        ...
                                        <MainInfoRequest route={route} setValFunc={setValues} values={values} edit={edit} openInfo={openInfo} allInfo={newRequest}/>
                                    </div>
